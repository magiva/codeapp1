...
import { Search, FileText, AlertCircle, DollarSign, Calendar } from "lucide-react"
...
function QuoteDetailDialog({ quoteId, onClose }: { quoteId: string | null; onClose: () => void }) {
  const { data, isLoading, isError } = useQuoteDetail(quoteId)

  const displayName = data?.name || "Quote"
  const initials = (data?.name?.[0] ?? "?").toUpperCase()

  return (
    <Dialog open={!!quoteId} onOpenChange={(open) => { if (!open) onClose() }}>
          ...
          <div className="min-w-0">
            {isLoading
              ...
              : <>
                ...
                {data?.quotenumber && (
                  <p className="text-zinc-400 text-sm truncate">Quote {data.quotenumber}</p>
                )}
              </>
            }
          </div>
          ...
          {data && (
            <div className="space-y-4">
              <div className="space-y-3">
                <DetailRow icon={<DollarSign className="h-4 w-4" />} label="Total Amount" value={data.totallineitemamount ? `$${data.totallineitemamount.toLocaleString()}` : null} />
                <DetailRow icon={<Calendar className="h-4 w-4" />} label="Expires" value={data.expireson ? new Date(data.expireson).toLocaleDateString() : null} />
              </div>
              {data.description && (
                <>
                  <Separator />
...
function QuoteCard({ quote, onClick }: { quote: Quotes; onClick: () => void }) {
  return (
    <Card
      onClick={onClick}
        ...
        <div className="flex items-center gap-3">
          <div className="h-10 w-10 rounded-full bg-[#D32C1E] flex items-center justify-center shrink-0">
            <span className="text-white text-sm font-semibold select-none">
              {(quote.name?.[0] ?? "?").toUpperCase()}
            </span>
          </div>
          <div className="min-w-0">
            <p className="font-semibold text-zinc-900 truncate leading-tight">{quote.name}</p>
            {quote.quotenumber && (
              <p className="text-xs text-zinc-500 truncate">Quote {quote.quotenumber}</p>
            )}
          </div>
        </div>
        ...
        <div className="flex flex-col gap-1.5 text-sm text-zinc-600">
          {quote.totallineitemamount && (
            <div className="flex items-center gap-2">
              ...
              <span className="font-medium">${quote.totallineitemamount.toLocaleString()}</span>
            </div>
          )}
          {quote.expireson && (
            <div className="flex items-center gap-2">
...
export function QuotesView() {
  const { filtered, isLoading, isError, error, search, setSearch } = useQuotes()
